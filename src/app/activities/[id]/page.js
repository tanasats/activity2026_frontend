...
export default function ActivityDetailPage() {
  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      toast.success('สมัครเข้าร่วมสำเร็จ!');
      // Reload or update state to show current status if needed
    } catch (error) {
      toast.error(error.response?.data?.message || 'สมัครเข้าร่วมไม่สำเร็จ');
    } finally {
                ...
                <div className="pt-6">
                  ...
                  >
                    {registering ? 'กำลังดำเนินการ...' : (
                      user?.role === 'student'
                        ? (activity.max_participants > 0 && activity.registered_count >= activity.max_participants ? 'กิจกรรมเต็มแล้ว' : 'สมัครเข้าร่วมกิจกรรม')
                        : (user ? 'เข้าสู่ระบบด้วยบัญชีนิสิต' : 'เข้าสู่ระบบเพื่อสมัคร')
                    )}
                  </Button>
              ...
              <div className="flex items-start space-x-3">
                ...
                <div>
                  ...
                  <p className="text-[10px] text-muted-foreground font-medium leading-relaxed">
                    กรุณามาให้ตรงตามเวลาที่นัดหมาย หากไม่สามารถเข้าร่วมได้ควรยกเลิกการสมัครเข้าร่วมล่วงหน้าอย่างน้อย 24 ชั่วโมง
                  </p>
                </div>
              </div>
            </div>
